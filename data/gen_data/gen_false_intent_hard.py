...
#         提取当前intent，根据CLUSTER随机找到一个*属于其category的intent new*，(若cluster只有其一个，跳过生成)
...
#         Finalise 错误数据
#         # {"category": "False_Intent_Hard", "query": "{New query}", "intent": [{"name": "{false intent}", "arguments": {"{name}": "...", "{name}": "..."}}], "match": "False"}
#     保存这一数据，检查好格式无误后append入false_intent_hard.jsonl
...
def pick_false_intent(original_intent_name):
    """
    Pick a false intent from the SAME cluster (excluding the original intent).
    Returns None if the cluster has only one intent (i.e., no other intents available).
    """
    orig_cluster = intent_to_cluster(original_intent_name)
    if orig_cluster is None:
        # Intent not found in any cluster
        return None
    
    # Get all intents in the same cluster, excluding the original
    same_cluster_intents = [
        intent for intent in CLUSTER[orig_cluster] 
        if intent != original_intent_name
    ]
    
    # If no other intents in the same cluster, return None (skip this data generation)
    if not same_cluster_intents:
        return None
    
    # Randomly pick one from the same cluster
    chosen_intent = random.choice(same_cluster_intents)
...
def generate_adversarial_data(
    ...
    output_jsonl_path="false_intent_hard.jsonl",
    model="gpt-4o",
    limit=None
):
    """
    ...
    for i, pos_item in enumerate(positive_data):
        ...
        try:
            ...
            # 3. Pick a false intent from the SAME cluster (excluding original)
            false_intent_name = pick_false_intent(original_intent_name)
            if false_intent_name is None:
                print(f"  [SKIP] No other intents in the same cluster for {original_intent_name}")
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="false_intent_hard.jsonl", help="Path to output JSONL")
